...
5. Preencha os campos com os seguintes valores:
   - **Servidor (Railway/produção)**: `rtmp://reseau.proxy.rlwy.net:50073/live`
   - **Servidor (Docker local)**: `rtmp://127.0.0.1:1935/live`
...
3. Fique de olho no site da sua TV no navegador:
   - produção: `https://tv-carlos-web-production.up.railway.app`;
   - Docker local: `http://localhost:8082`.
...
* **O OBS dá erro ao tentar conectar:**
  - Em produção, confirme o endpoint `rtmp://reseau.proxy.rlwy.net:50073/live` e se o serviço Railway está saudável.
  - Em ambiente local, verifique os containers com `docker compose ps` e use `rtmp://127.0.0.1:1935/live`.
  - Confirme que a chave contém `stream?token=` seguida do token correto.

## Segurança em produção

O endpoint Railway usa TCP Proxy público e exige o token validado pela API. A chave deve permanecer somente no OBS e no secret manager do Railway. Rotacione `RTMP_STREAM_KEY` imediatamente em caso de vazamento e nunca publique a chave em commits, prints ou mensagens públicas.
